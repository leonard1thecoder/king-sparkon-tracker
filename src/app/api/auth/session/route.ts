...
type ExtendedClaims = NonNullable<ReturnType<typeof decodeJwtPayload>> & {
  name?: string;
  username?: string;
  email?: string;
};
...
export async function GET() {
  ...
  const claims = decodeJwtPayload(cookieStore.get(ACCESS_COOKIE_NAME)?.value) as ExtendedClaims | null;
