...
function backendHeaders(request: NextRequest) {
  ...
  // Preserve the idempotency key sent by the frontend.
...
async function proxy(request: NextRequest, path: string[]) {
  ...
  if (!request.cookies.get(AUTH_COOKIE_NAME)?.value) {
    return Response.json(
      {
        message:
          "Sign in before calling protected backend resources.",
      },
      { status: 401 },
    );
  }
  ...
  const backendUrl = new URL(
    `/api/${backendPath}${incomingUrl.search}`,
    backendBaseUrl(),
  );
    ...
    return new Response(text, {
      ...
      headers: {
        ...
        ...(backendResponse.headers.get("retry-after")
          ? {
              "retry-after":
                backendResponse.headers.get("retry-after") as string,
            }
          : {}),
      },
    });
