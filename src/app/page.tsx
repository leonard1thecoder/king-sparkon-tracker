...
export const metadata: Metadata = {
  title: "King Sparkon Tracker | Barcode Inventory, Tickets, Jobs, Tips & Payments",
  description:
    "Premium barcode, QR inventory, job opportunity, and event ticket management software for stock scanning, QR ticket verification, worker tips, affiliate programs, payments, reports, and audit-ready operations.",
  keywords: [
  ...
  openGraph: {
    ...
    description: "Barcode operations, job opportunities, and QR ticket verification platform for inventory, payments, tips, affiliates, reports, and event entry teams.",
    type: "website",
  ...
  twitter: {
    ...
    description: "Scan, track, verify, post jobs, sell tickets, and report from one premium barcode operations dashboard.",
...
const heroBadges = ["Barcode scanning", "QR tickets", "Job opportunities", "Owner reports", "Worker tips", "Affiliate referrals"] as const;
const trustedTeams = ["Retail", "Warehouse", "Field Teams", "Small Business", "Events", "Logistics"] as const;

const workflows = [
  ["01", "Add", "Add products, workers, events, ticket classes, job openings, affiliate links, and business rules into one workspace."],
  ["02", "Generate", "Create barcode labels, ticket QR codes, worker tip QR cards, referral links, job posts, and payment URLs."],
  ["03", "Scan", "Scan at the counter, shelf, event gate, delivery point, or tip stand with manual fallback when needed."],
  ["04", "Report", "Give owners capacity, stock, job applications, scan activity, payment, class sales, payout, and audit history visibility."],
] as const;
...
const features = [
  ...
  { icon: BriefcaseBusiness, title: "Job opportunities", copy: "Businesses can publish open roles, users can apply, and owners or admins can review application status from the dashboard.", tags: ["Open roles", "Applications", "Role-aware"] },
...
const jobHighlights = [
  ["Browse open roles", "Users can discover OPEN job opportunities from the public page or their role dashboard."],
  ["Apply in-app", "Applicants submit name, email, phone, cover message, and CV link through a production-ready form."],
  ["Manage postings", "Owners and admins can create, publish, close, archive, and inspect job opportunities."],
  ["Review applications", "Application status stays visible for owners, admins, and users tracking their own submissions."],
] as const;
...
const dashboardPreviewCards: Array<{ icon: LucideIcon; label: string; value: string }> = [
  ...
  { icon: BriefcaseBusiness, label: "Job posts", value: "Ready" },
...
export default function MarketingPage() {
  return (
    <main className="bg-white text-[var(--ink)]">
      <section className="relative overflow-hidden bg-white pt-24 enterprise-grid">
        ...
        <header className="fixed inset-x-0 top-0 z-50 border-b border-[var(--line)] bg-white/88 shadow-[0_18px_60px_rgba(7,19,31,0.08)] backdrop-blur-xl">
          <div className="border-b border-[var(--line)] bg-[var(--ink)] px-5 py-2 text-center text-xs font-black text-white/78">
            <span className="text-[var(--gold)]">New:</span> real-time barcode tracking, job opportunities, QR tickets, worker tips, and owner dashboard clarity.
          </div>
          <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 md:px-8" aria-label="Primary navigation">
            ...
            <div className="hidden items-center gap-6 text-sm font-semibold text-[var(--steel)] lg:flex">
              ...
              <a href="#jobs" className="hover:text-[var(--ink)]">Jobs</a>
            ...
            <div className="flex items-center gap-2">
              <Link href="/jobs" className="hidden min-h-11 items-center justify-center rounded-full border border-[var(--line)] px-4 text-sm font-bold text-[var(--steel)] hover:border-[var(--gold)] hover:text-[var(--ink)] md:inline-flex">Jobs</Link>
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl gap-12 px-5 pb-16 pt-12 md:px-8 lg:grid-cols-[1.02fr_0.98fr] lg:pb-24 lg:pt-18">
          <div className="flex flex-col justify-center">
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--steel)]">King Sparkon Tracker brings scanning, stock movement, QR event tickets, job opportunities, worker tips, affiliate referrals, payments, promotions, and audit-ready reporting into one enterprise-grade business workspace.</p>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <Link href="/jobs" className="inline-flex min-h-12 items-center justify-center rounded-full border border-[var(--line)] bg-white px-6 font-bold text-[var(--ink)] shadow-[var(--shadow-soft)] hover:border-[var(--gold)]">Explore jobs</Link>
            </div>
          </div>
          <div className="grid gap-5">
            <ScanLoop />
            <div className="grid gap-3 rounded-[2rem] border border-[var(--line)] bg-white/82 p-4 shadow-[var(--shadow-ledger)] backdrop-blur sm:grid-cols-4">
              {["Inventory", "Tickets", "Jobs", "Tips"].map((item) => <div key={item} className="rounded-[1.35rem] border border-[var(--line)] bg-[var(--surface)] p-4"><p className="font-mono text-[0.65rem] font-black uppercase tracking-[0.16em] text-[var(--signal)]">{item}</p><p className="mt-2 text-sm font-bold text-[var(--steel)]">Ready for dashboard visibility</p></div>)}
            </div>
          </div>
        </div>
      ...
      <section id="jobs" className="scroll-mt-28 px-5 py-16 md:px-8 lg:py-24"><div className="mx-auto grid max-w-7xl gap-8 overflow-hidden rounded-[2.5rem] border border-[var(--line)] bg-[var(--surface)] p-6 shadow-[var(--shadow-ledger)] lg:grid-cols-[0.92fr_1.08fr] lg:p-8"><div><p className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-[var(--signal)]">05 / job opportunities</p><h2 className="mt-4 text-4xl font-black tracking-[-0.05em] md:text-6xl">Roles, applications, and users in the same ecosystem.</h2><p className="mt-5 text-sm leading-7 text-[var(--steel)] md:text-base">Businesses can post opportunities, users can apply, and owners or admins can review applicants without breaking role-based navigation.</p><div className="mt-7 flex flex-col gap-3 sm:flex-row"><Link href="/jobs" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[var(--signal)] bg-[var(--signal)] px-6 font-bold text-white shadow-[var(--shadow-soft)] hover:bg-[var(--ink)]">Explore jobs <ArrowRight className="h-4 w-4" /></Link><Link href="/dashboard/owner/jobs" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[var(--line)] bg-white px-6 font-bold text-[var(--ink)] shadow-[var(--shadow-soft)] hover:border-[var(--gold)]"><BriefcaseBusiness className="h-4 w-4" /> Post a job</Link></div></div><div className="grid gap-4 sm:grid-cols-2">{jobHighlights.map(([title, copy]) => <article key={title} className="rounded-[1.75rem] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-soft)]"><BriefcaseBusiness className="h-5 w-5 text-[var(--signal)]" /><h3 className="mt-5 text-xl font-black tracking-[-0.03em]">{title}</h3><p className="mt-3 text-sm leading-6 text-[var(--steel)]">{copy}</p></article>)}</div></div></section>

      <section id="fees" className="scroll-mt-28 px-5 py-16 md:px-8 lg:py-24"><div className="mx-auto grid max-w-7xl gap-6 rounded-[2.5rem] border border-[var(--line)] bg-[var(--surface)] p-6 shadow-[var(--shadow-soft)] lg:grid-cols-[0.8fr_1.2fr]"><div><p className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-[var(--signal)]">06 / fee visibility</p><h2 className="mt-4 text-4xl font-black tracking-[-0.05em] md:text-5xl">No hidden payout maths.</h2><p className="mt-4 text-sm leading-7 text-[var(--steel)]">The UI keeps platform retention, gross totals, net amount, and payout state visible for ticket owners and worker tips.</p></div><div className="grid gap-4 md:grid-cols-2">{feeHighlights.map(([title, copy]) => <article key={title} className="rounded-[1.75rem] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-soft)]"><CreditCard className="h-5 w-5 text-[var(--signal)]" /><h3 className="mt-5 text-xl font-black tracking-[-0.03em]">{title}</h3><p className="mt-3 text-sm leading-6 text-[var(--steel)]">{copy}</p></article>)}</div></div></section>

      <section id="affiliate" className="scroll-mt-28 px-5 py-16 md:px-8 lg:py-24"><div className="mx-auto grid max-w-7xl gap-8 rounded-[2.5rem] border border-[var(--line)] bg-white p-6 shadow-[var(--shadow-ledger)] lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:p-8"><div><p className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-[var(--signal)]">07 / affiliate program</p><h2 className="mt-4 text-4xl font-black tracking-[-0.05em] md:text-6xl">Turn promoters into measurable growth.</h2><p className="mt-5 text-sm leading-7 text-[var(--steel)] md:text-base">Referral links, QR previews, commission visibility, payout tracking, and marketing assets for promoters.</p><Link href="/register-affiliate" className="mt-7 inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[var(--signal)] bg-[var(--signal)] px-6 font-bold text-white shadow-[var(--shadow-soft)] hover:bg-[var(--ink)]">Join Affiliate Program <ArrowRight className="h-4 w-4" /></Link><div className="mt-8 rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-5"><div className="barcode-rule text-[var(--ink)]" /><p className="mt-4 text-sm leading-7 text-[var(--steel)]">Commission tiers: {AFFILIATE_COMMISSION_SUMMARY}.</p></div></div><div className="grid gap-4 sm:grid-cols-2">{affiliate.map(([title, copy]) => <article key={title} className="rounded-[1.75rem] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-[var(--shadow-soft)]"><CheckCircle2 className="h-5 w-5 text-[var(--signal)]" /><h3 className="mt-5 text-xl font-black tracking-[-0.03em]">{title}</h3><p className="mt-3 text-sm leading-6 text-[var(--steel)]">{copy}</p></article>)}</div></div></section>

      <section id="pricing" className="scroll-mt-28 px-5 py-16 md:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div className="max-w-3xl"><p className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-[var(--signal)]">08 / pricing</p><h2 className="mt-4 text-4xl font-black tracking-[-0.05em] md:text-6xl">Clear plans from one policy mirror.</h2><p className="mt-4 text-sm leading-7 text-[var(--steel)] md:text-base">Plan cards read from shared config and can later move to the billing API. Register keeps the selected service visible.</p></div><Link href="/register?service=FULL_BUSINESS_SUITE" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[var(--signal)] bg-[var(--signal)] px-6 font-bold text-white shadow-[var(--shadow-soft)] hover:bg-[var(--ink)]">Start free trial <ArrowRight className="h-4 w-4" /></Link></div><div className="mt-10 grid gap-4 lg:grid-cols-3">{BUSINESS_PRICING_PLANS.map((plan) => <article key={plan.name} className={`relative overflow-hidden rounded-[2rem] border p-6 shadow-[var(--shadow-soft)] ${plan.highlight ? "border-[var(--gold)] bg-[var(--surface)]" : "border-[var(--line)] bg-white"}`}>{plan.highlight ? <span className="absolute right-5 top-5 rounded-full bg-[var(--ink)] px-3 py-1 text-xs font-black uppercase tracking-[0.12em] text-[var(--gold)]">Recommended</span> : null}<p className="font-mono text-xs font-black uppercase tracking-[0.16em] text-[var(--signal)]">{plan.caption}</p><h3 className="mt-3 text-3xl font-black tracking-[-0.05em]">{plan.name}</h3><p className="money mt-4 text-4xl font-black">{plan.priceDisplay}<span className="text-sm font-bold text-[var(--steel)]">{plan.billingSuffix ? ` / ${plan.billingSuffix}` : ""}</span></p><p className="mt-4 text-sm leading-7 text-[var(--steel)]">{plan.description}</p><ul className="mt-5 grid gap-2">{plan.features.map((feature) => <li key={feature} className="flex gap-2 text-sm font-bold text-[var(--steel)]"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[var(--signal)]" />{feature}</li>)}</ul><Link href={`/register?plan=${planQuery(plan.name)}&service=FULL_BUSINESS_SUITE`} className="mt-6 inline-flex min-h-11 w-full items-center justify-center rounded-full border border-[var(--signal)] bg-[var(--signal)] px-4 text-sm font-black text-white hover:bg-[var(--ink)]">Choose {plan.name}</Link></article>)}</div></div></section>

      <section id="contact" className="mx-auto max-w-7xl scroll-mt-28 px-5 py-16 md:px-8 lg:py-24"><div className="grid overflow-hidden rounded-[2.25rem] border border-[var(--line)] bg-white shadow-[var(--shadow-ledger)] lg:grid-cols-[0.82fr_1.18fr]"><div className="bg-[var(--ink)] p-6 text-white enterprise-grid md:p-8"><p className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-[var(--gold)]">09 / contact</p><h2 className="mt-4 max-w-xl break-words text-3xl font-black leading-[1.02] tracking-[-0.05em] sm:text-4xl lg:text-5xl">Send an implementation inquiry.</h2><p className="mt-4 max-w-xl text-sm leading-7 text-white/68">Tell us what you track, where workers scan, which jobs users apply to, which events need QR entry, and which reports owners need first.</p><div className="barcode-rule mt-8 max-w-md text-white" /></div><div className="p-5 md:p-8"><ContactForm /></div></div></section>
    </main>
  );
}
