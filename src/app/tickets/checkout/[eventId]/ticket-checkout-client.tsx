      ...
      <main className="mx-auto max-w-7xl px-5 py-16 md:px-8 lg:py-24">
        ...
        {!isLoading && !event ? (
          <div className="rounded-[2rem] border border-dashed border-[var(--line-strong)] bg-white p-10 text-center shadow-[var(--shadow-soft)]">
            ...
            <Link href="/tickets" className="mt-5 inline-flex min-h-12 items-center justify-center rounded-full border border-[var(--signal)] bg-[var(--signal)] px-5 text-sm font-black text-white">Back to tickets</Link>
          </div>
        ) : null}
          ...
          <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-[1fr_0.78fr]">
            <section className="rounded-[2.35rem] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-ledger)] md:p-8">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  <p className="font-mono text-xs font-black uppercase tracking-[0.18em] text-[var(--signal)]">Ticket checkout</p>
                  <h1 className="mt-3 text-4xl font-black tracking-[-0.05em] md:text-5xl">Buy tickets for {event.name}</h1>
                  <p className="mt-3 text-sm leading-7 text-[var(--steel)]">Typed payment placeholder. Swap this call to Stripe, Yoco, Ozow, or backend checkout when the API is ready.</p>
                </div>
