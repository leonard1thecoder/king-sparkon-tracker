...
export function EventDetailsClient({ eventId }: EventDetailsClientProps) {
  ...
  if (isLoading) {
    return <TicketLayout><main className="mx-auto max-w-7xl px-5 py-16 md:px-8"><div className="h-[32rem] animate-pulse rounded-[2.4rem] border border-[var(--line)] bg-[var(--surface)]" /></main></TicketLayout>;
  }

  if (!event) {
    return <TicketLayout><main className="mx-auto max-w-4xl px-5 py-20 text-center md:px-8"><div className="rounded-[2rem] border border-dashed border-[var(--line-strong)] bg-white p-10 shadow-[var(--shadow-soft)]"><Ticket className="mx-auto h-10 w-10 text-[var(--signal)]" /><h1 className="mt-4 text-3xl font-black tracking-[-0.04em]">Event not found</h1><p className="mt-2 text-sm font-semibold text-[var(--steel)]">This event may have been removed or the ID is incorrect.</p><Link href="/tickets" className="mt-5 inline-flex min-h-12 items-center justify-center rounded-full border border-[var(--signal)] bg-[var(--signal)] px-5 text-sm font-black text-white">Back to tickets</Link></div></main></TicketLayout>;
  }
