...
export function TicketsLandingClient() {
      ...
      <main className="bg-white">
        <section className="relative overflow-hidden px-5 py-16 md:px-8 lg:py-24">
          ...
          <div className="relative z-10 mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-[var(--line)] bg-white px-3 py-2 font-mono text-[0.68rem] font-black uppercase tracking-[0.16em] text-[var(--steel)] shadow-[var(--shadow-soft)]"><span className="h-2 w-2 rounded-full bg-[var(--confirm)]" /> QR ticket portal</div>
              <h1 className="mt-6 max-w-5xl text-5xl font-black leading-[0.96] tracking-[-0.07em] md:text-7xl">Find, Buy & Verify Tickets with King Sparkon Tracker</h1>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--steel)]">Extend the barcode identity into serious event ticketing: sell ticket classes, track capacity, show user QR tickets, and verify entry at the gate.</p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Link href="/tickets/my-tickets" className="inline-flex min-h-12 items-center justify-center rounded-full border border-[var(--line)] bg-white px-6 font-black text-[var(--ink)] shadow-[var(--shadow-soft)] hover:border-[var(--signal)]">My Tickets</Link>
              </div>
            </div>
            <div className="rounded-[2.4rem] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-depth)]">
              <div className="overflow-hidden rounded-[2rem] bg-[var(--ink)] p-6 text-white">
                <div className="flex items-start justify-between"><div><p className="font-mono text-xs font-black uppercase tracking-[0.18em] text-[var(--gold)]">Live gate scanner</p><h2 className="mt-3 text-3xl font-black tracking-[-0.05em]">Ticket QR verification that feels professional.</h2></div><QrCode className="h-8 w-8 text-[var(--gold)]" /></div>
        ...
        <section id="events" className="mx-auto max-w-7xl scroll-mt-28 px-5 pb-16 md:px-8 lg:pb-24">
          ...
          {isLoading ? (
            <div className="mt-8 grid gap-4 md:grid-cols-3">{[0, 1, 2].map((item) => <div key={item} className="h-96 animate-pulse rounded-[2rem] border border-[var(--line)] bg-[var(--surface)]" />)}</div>
          ) : filteredEvents.length > 0 ? (
            <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{filteredEvents.map((event) => <TicketEventCard key={event.id} event={event} />)}</div>
          ) : (
            <div className="mt-8 rounded-[2rem] border border-dashed border-[var(--line-strong)] bg-white p-10 text-center shadow-[var(--shadow-soft)]"><Ticket className="mx-auto h-10 w-10 text-[var(--signal)]" /><h2 className="mt-4 text-2xl font-black tracking-[-0.04em]">No events match your filters</h2><p className="mt-2 text-sm font-semibold leading-6 text-[var(--steel)]">Clear search filters or create a new event from the owner dashboard.</p><Link href="/tickets/owner/create" className="mt-5 inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[var(--signal)] bg-[var(--signal)] px-5 text-sm font-black text-white shadow-[var(--shadow-soft)] hover:bg-[var(--ember)]"><ShieldCheck className="h-4 w-4" /> Create event</Link></div>
          )}
        </section>
      </main>
    </TicketLayout>
