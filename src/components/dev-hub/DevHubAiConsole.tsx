    ...
    <section className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
        <form onSubmit={(event) => void submitRequest(event)} className="rounded-[2rem] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-soft)]">
          <div className="flex items-center gap-2 text-[var(--signal)]"><Code2 className="h-5 w-5" /><p className="font-mono text-xs font-black uppercase tracking-[0.22em]">Send request</p></div>
          ...
          <button disabled={isSubmitting} className="mt-4 inline-flex h-12 items-center justify-center gap-2 rounded-full bg-[var(--gold)] px-6 text-sm font-black text-[var(--ink)] disabled:opacity-60">{isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}Get AI price and plan</button>
        </form>
        ...
        <div className="rounded-[2rem] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-soft)]">
          ...
          {createdRequest ? <div className="mt-4 space-y-4"><h2 className="text-2xl font-black tracking-[-0.04em] text-[var(--ink)]">{createdRequest.currency} {createdRequest.estimatedMinPrice} - {createdRequest.estimatedMaxPrice}</h2><p className="text-sm font-bold leading-7 text-[var(--muted)]">{createdRequest.aiAutomatedResponse}</p><div className="rounded-3xl bg-[var(--surface)] p-4 text-sm font-semibold text-[var(--steel)]">{createdRequest.aiDevelopmentPlan}</div><span className="inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">{createdRequest.status}</span></div> : <p className="mt-4 text-sm font-bold leading-7 text-[var(--muted)]">Submit a request to generate an instant price range and development plan.</p>}
        </div>
      </div>
      ...
      <div className="rounded-[2rem] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-soft)]">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-6">
          ...
          <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="h-11 rounded-full border border-[var(--line)] px-4 text-sm font-semibold"><option value="createdAt">Created</option><option value="clientName">Client</option><option value="companyName">Company</option><option value="projectType">Project type</option><option value="estimatedMinPrice">Min price</option><option value="estimatedMaxPrice">Max price</option></select>
