...
export function MobileBottomNav({ role: rawRole }: { role: string }) {
  ...
  return (
    <>
      <nav
        className="fixed bottom-0 left-0 right-0 z-40 border-t border-[var(--line)] bg-white/95 backdrop-blur-md shadow-[0_-4px_20px_rgba(0,0,0,0.06)] pb-[env(safe-area-inset-bottom)] lg:hidden"
          ...
          {primary.map(({ label, shortLabel, href, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "group flex flex-col items-center justify-center h-full w-full py-1 text-center transition-colors duration-200 relative min-h-[44px]",
              ...
              >
                ...
                <span className={cn(
                  "grid h-7 w-7 place-items-center rounded-[var(--radius-md)] transition-all duration-200 group-active:scale-95",
      ...
      {moreOpen && hasMore ? (
        <>
          ...
          />
          <div
            className="fixed inset-x-0 bottom-0 z-[60] flex max-h-[85vh] flex-col rounded-t-[var(--radius-2xl)] border-t border-[var(--line)] bg-white p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.18)] pb-[calc(1.5rem+env(safe-area-inset-bottom))] lg:hidden animate-in slide-in-from-bottom duration-200"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="flex shrink-0 items-center justify-between gap-3 border-b border-[var(--line)] pb-3">
              <div className="flex items-center gap-2">
                <span className="rounded-[var(--radius-sm)] border border-[var(--line)] bg-[var(--signal-soft)] px-2.5 py-1 text-[0.65rem] font-extrabold uppercase tracking-[0.1em] text-[var(--signal-strong)]">
                  {role} Menu
                </span>
              ...
              <button
                type="button"
                onClick={() => setMoreOpen(false)}
                className="grid h-9 w-9 place-items-center rounded-full border border-[var(--line)] text-[var(--steel)] hover:bg-[var(--surface)] hover:text-[var(--ink)] transition-colors duration-200"
              ...
              {allSecondaryItems.map(({ label, href, icon: Icon, description }) => {
                ...
                return (
                  <Link
                    ...
                    aria-current={active ? "page" : undefined}
                    className={cn(
                      "group flex min-h-[3.25rem] w-full items-center justify-between rounded-[var(--radius-lg)] border px-3.5 py-2.5 transition-colors duration-200",
                      active
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <span className={cn(
                        "grid h-8 w-8 shrink-0 place-items-center rounded-[var(--radius-md)] border transition-colors duration-200",
                    ...
                    <ChevronRight className={cn("h-4 w-4 shrink-0 transition-transform duration-200 group-hover:translate-x-0.5", active ? "text-[var(--signal-strong)]" : "text-[var(--steel)]")} />
                  </Link>
                );
              })}
            </div>
            ...
            <div className="mt-2 grid shrink-0 gap-2 border-t border-[var(--line)] pt-3">
              <Link
                href={profileHref}
                onClick={() => setMoreOpen(false)}
                className="flex min-h-11 w-full items-center justify-center gap-2 rounded-[var(--radius-md)] border border-[var(--line)] bg-white text-xs font-black text-[var(--ink)] hover:bg-[var(--surface)] transition-colors duration-200"
              >
                <UserRound className="h-4 w-4 text-[var(--signal)]" /> Update Account Profile
              </Link>
              <LogoutButton
                className="flex min-h-11 w-full items-center justify-center gap-2 rounded-[var(--radius-md)] border border-[var(--danger)]/30 bg-[var(--danger)]/5 text-xs font-black text-[var(--danger)] hover:bg-[var(--danger)] hover:text-white transition-colors duration-200"
                ariaLabel="Sign out"
              >
