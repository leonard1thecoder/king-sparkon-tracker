...
function resolveDiameter(
  ...
  if (orientation === "vertical") {
    return clamp(Math.min(config.mobileDiameter, width - 88), 118, config.mobileDiameter);
  }
    ...
    >
      ...
      >
        <path
          ...
          fill="none"
          stroke="var(--signal)"
          strokeWidth={isSubscription ? "10" : "12"}
          strokeLinecap="round"
        ...
        <path
          ...
          fill="none"
          stroke="var(--signal)"
          strokeWidth={isSubscription ? "3.25" : "4"}
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeDasharray={isSubscription ? "16 11" : "20 12"}
      ...
      {items.map(({ icon: Icon, eyebrow, title, copy, tags }, index) => (
        <article
          ...
          className={`absolute left-0 top-0 z-10 flex aspect-square min-h-0 will-change-transform flex-col items-center justify-center rounded-full border border-[var(--line-strong)] bg-white text-center shadow-[0_18px_45px_rgba(14,165,233,0.13)] ${classes.bubble}`}
        >
          <div
            className={`grid shrink-0 place-items-center rounded-full border border-[var(--line)] bg-[var(--signal-soft)] text-[var(--signal)] ${isSubscription ? "h-9 w-9" : "h-10 w-10 sm:h-11 sm:w-11"}`}
          >
            <Icon className={isSubscription ? "h-4 w-4" : "h-5 w-5"} />
          </div>
          <p
            className={`font-extrabold uppercase text-[var(--signal-strong)] ${isSubscription ? "mt-2 text-[0.46rem] tracking-[0.09em]" : "mt-2.5 text-[0.54rem] tracking-[0.11em]"}`}
          >
            {eyebrow ?? `0${index + 1}`}
          </p>
          <h3
            className={`text-balance font-black leading-tight tracking-[-0.03em] ${isSubscription ? "mt-1 text-sm" : "mt-1 text-sm sm:text-base"}`}
          >
            {title}
          </h3>
          <p
            className={`mt-1.5 text-balance text-[var(--steel)] ${variant === "features" ? "text-[0.58rem] leading-[0.86rem] sm:text-[0.64rem] sm:leading-4" : isSubscription ? "text-[0.56rem] leading-[0.82rem]" : "text-[0.66rem] leading-[0.98rem] sm:text-[0.7rem] sm:leading-4"}`}
          >
            {copy}
          </p>
          {tags?.length ? (
            <div className="mt-2 flex max-w-full flex-wrap justify-center gap-1">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-[var(--line)] bg-[var(--signal-soft)] px-1.5 py-0.5 text-[0.46rem] font-extrabold text-[var(--signal-strong)] sm:text-[0.5rem]"
                >
                  {tag}
                </span>
              ))}
