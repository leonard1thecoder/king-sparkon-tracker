...
const pricingPlans = [
  { name: "Free User", price: "R0", caption: "Forever", href: "/register?plan=FREE_USER&privilege=USER&service=FREE_USER_ACCESS", highlight: false, copy: "For customers and applicants.", features: ["Free account", "Browse job opportunities", "Apply for jobs", "Buy and view QR tickets", "Cart-ready checkout", "Profile and purchase QR access"] },
  { name: "Free Affiliate", price: "R0", caption: "Forever", href: "/register?plan=FREE_AFFILIATE&privilege=AFFILIATE&service=FREE_AFFILIATE_ACCESS", highlight: false, copy: "For promoters and referral partners.", features: ["Free affiliate account", "Referral link shell", "QR promotion cards", "Campaign assets", "Commission visibility", "Affiliate dashboard"] },
  { name: "Free Trial Business", price: "R0", caption: "14 days", href: "/register?plan=FREE_TRIAL_BUSINESS&privilege=BUSINESS_OWNER&service=FULL_BUSINESS_SUITE", highlight: false, copy: "For owners testing the full stack.", features: ["2 workers", "Unlimited products", "Unlimited scans", "QR ticket preview", "Job post preview", "Core owner dashboard"] },
  { name: "Plus", price: "R880", caption: "per month", href: "/register?plan=PLUS&privilege=BUSINESS_OWNER&service=FULL_BUSINESS_SUITE", highlight: true, copy: "For growing stores and small teams.", features: ["5 workers", "Product and barcode scanning", "Cart and transactions", "QR tickets and capacity", "Jobs and applications", "Promotions and reports"] },
  { name: "Pro", price: "R2,300", caption: "per month", href: "/register?plan=PRO&privilege=BUSINESS_OWNER&service=FULL_BUSINESS_SUITE", highlight: false, copy: "For King-level operators.", features: ["Unlimited workers", "Tips and review flows", "Advanced reports", "Audit history", "Full jobs, tickets, cart, promotion, affiliate stack", "Priority implementation support"] },
] as const;
...
export function KingSparkonLanding() {
  return (
    <main className="bg-white text-[var(--ink)]">
      ...
      <section id="pricing" className="scroll-mt-28 px-5 py-16 md:px-8 lg:py-24"><div className="mx-auto max-w-7xl"><div className="max-w-3xl"><p className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-[var(--signal)]">04 / pricing</p><h2 className="mt-4 text-4xl font-black tracking-[-0.055em] md:text-6xl">Free where it must be free. Paid where business gets power.</h2><p className="mt-4 text-sm leading-7 text-[var(--steel)] md:text-base">Prices include the feature set. User and Affiliate start at R0.</p></div><div className="mt-10 grid gap-4 lg:grid-cols-5">{pricingPlans.map((plan) => <article key={plan.name} className={`relative overflow-hidden rounded-[2rem] border p-5 shadow-[var(--shadow-soft)] ${plan.highlight ? "border-[var(--gold)] bg-[var(--surface)]" : "border-[var(--line)] bg-white"}`}>{plan.highlight ? <span className="absolute right-4 top-4 rounded-full bg-[var(--ink)] px-3 py-1 text-xs font-black uppercase tracking-[0.12em] text-[var(--gold)]">Best grow</span> : null}<p className="font-mono text-xs font-black uppercase tracking-[0.16em] text-[var(--signal)]">{plan.caption}</p><h3 className="mt-3 text-2xl font-black tracking-[-0.05em]">{plan.name}</h3><p className="money mt-4 text-4xl font-black">{plan.price}</p><p className="mt-4 text-sm leading-7 text-[var(--steel)]">{plan.copy}</p><ul className="mt-5 grid gap-2">{plan.features.map((feature) => <li key={feature} className="flex gap-2 text-sm font-bold text-[var(--steel)]"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[var(--signal)]" />{feature}</li>)}</ul><Link href={plan.href} className="mt-6 inline-flex min-h-11 w-full items-center justify-center rounded-full border border-[var(--signal)] bg-[var(--signal)] px-4 text-sm font-black text-white hover:bg-[var(--ink)]">Choose {plan.name}</Link></article>)}</div></div></section>
