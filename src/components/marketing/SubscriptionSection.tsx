...
const inputClasses = "h-12 rounded-lg border border-[var(--line)] bg-white px-4 text-[var(--ink)] shadow-[var(--shadow-soft)] outline-none placeholder:text-[var(--muted)] hover:border-[var(--signal)] focus:border-[var(--signal)] focus:ring-4 focus:ring-[rgba(14,165,233,0.14)]";
...
const subscriptionNotes: readonly BouncingCircleItem[] = [
  { icon: ShieldCheck, title: "Build notes", copy: "What changed and why it matters." },
  { icon: Mail, title: "Feature drops", copy: "New scanner, QR, and dashboard releases." },
  { icon: Crown, title: "Founder signal", copy: "Product direction from the King Sparkon roadmap." },
];
...
function statusClasses(tone: SubscriptionStatus["tone"]) {
  if (tone === "success") return "border-[var(--confirm)]/40 bg-[var(--confirm)]/10 text-[var(--confirm)]";
  if (tone === "warning") return "border-[var(--signal)]/40 bg-[var(--signal-soft)] text-[var(--signal-strong)]";
        ...
        <div className="relative border border-[var(--line-strong)] bg-white p-5 shadow-[var(--shadow-soft)] md:p-8">
          <div className="relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[0.62rem] font-black uppercase tracking-[0.16em] text-[var(--signal-strong)]">King Sparkon inbox</p>
            ...
            <form className="mt-7 grid gap-4" onSubmit={handleSubmit}>
              ...
              <div className="grid gap-3 border border-[var(--line)] bg-white p-4">
                <p className="text-[0.65rem] font-black uppercase tracking-[0.14em] text-[var(--signal-strong)]">Subscribe as</p>
                <div className="grid gap-2 sm:grid-cols-2">
                  {subscribeAsOptions.map((option) => (
                    <button key={option} type="button" onClick={() => setSubscribeAs(option)} className={`min-h-11 rounded-lg border px-4 text-xs font-black transition ${toggleClasses(option === subscribeAs)}`} aria-pressed={option === subscribeAs}>
                      {option}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid gap-3 border border-[var(--line)] bg-white p-4">
                <p className="text-[0.65rem] font-black uppercase tracking-[0.14em] text-[var(--signal-strong)]">Interested in</p>
                <div className="grid gap-2 sm:grid-cols-2">
                  {interestOptions.map((option) => (
                    <button key={option} type="button" onClick={() => setInterest(option)} className={`min-h-11 rounded-lg border px-4 text-xs font-black transition ${toggleClasses(option === interest)}`} aria-pressed={option === interest}>
                      {option}
                    </button>
                  ))}
                </div>
              ...
              {status ? (
                <div className={`flex items-start gap-3 rounded-lg border px-4 py-3 text-sm font-semibold leading-6 ${statusClasses(status.tone)}`} role={status.tone === "error" ? "alert" : "status"}>
