...
export function DashboardMyTickets() {
  ...
  useEffect(() => {
    ...
    async function loadTickets() {
      try {
        ...
        const ticketsWithEvents = await Promise.all(nextTickets.map(async (ticket) => ({ ticket, event: await getLiveEventById(ticket.eventId) })));
        if (!mounted) return;
        ...
        if (!mounted) return;
        setItems([]);
        setError(loadError instanceof Error ? loadError.message : "Purchased tickets could not be loaded.");
      } finally {
  ...
  return (
    <>
      <DashboardHeader role="USER WORKSPACE" title="My purchased tickets" description="View QR tickets issued by the backend after Stripe payment and verified webhook fulfilment." />
      <main className="bg-[var(--surface)] p-5 md:p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div><p className="font-mono text-xs font-black uppercase tracking-[0.18em] text-[var(--signal)]">My tickets</p><h1 className="mt-3 text-5xl font-black tracking-[-0.06em] md:text-6xl">Verified ticket QR dashboard</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--steel)]">Every ticket here was issued after Stripe confirmation. Status, reference number, event details, and QR access are loaded from the backend.</p></div>
        ...
        {!isLoading && items.length === 0 ? (
          <div className="mt-8 rounded-[2rem] border border-dashed border-[var(--line-strong)] bg-white p-10 text-center shadow-[var(--shadow-soft)]"><Ticket className="mx-auto h-10 w-10 text-[var(--signal)]" /><h2 className="mt-4 text-3xl font-black tracking-[-0.04em]">You have no verified tickets yet</h2><p className="mt-2 text-sm font-semibold leading-6 text-[var(--steel)]">Complete a Stripe cart payment and the webhook-issued QR ticket will appear here.</p><Link href="/dashboard/user/tickets/buy" className="mt-5 inline-flex min-h-12 items-center justify-center rounded-full border border-[var(--signal)] bg-[var(--signal)] px-5 text-sm font-black text-white shadow-[var(--shadow-soft)]">Browse events</Link></div>
        ) : null}
