      ...
      <main className="grid gap-7 bg-[var(--surface)] p-5 md:p-8">
        <section className="relative overflow-hidden rounded-[2.5rem] border border-[var(--line)] bg-white p-5 shadow-[var(--shadow-ledger)] md:p-7">
          <div className="grid gap-8 lg:grid-cols-[0.92fr_1.08fr] lg:items-center">
            <div className="overflow-hidden rounded-[2.4rem] border border-[var(--line)] bg-[var(--ink)] shadow-[var(--shadow-depth)]"><div className="relative min-h-[28rem]">{event.bannerUrl ? <img src={event.bannerUrl} alt={`${event.name} banner`} className="absolute inset-0 h-full w-full object-cover opacity-86" loading="lazy" /> : <div className="absolute inset-0 scan-grid" />}<div className="absolute inset-0 bg-gradient-to-t from-[var(--ink)] via-[var(--ink)]/30 to-transparent" /><div className="absolute bottom-6 left-6 right-6"><div className="barcode-rule mb-6 text-white" /><TicketStatusBadge status={event.status} /></div></div></div>
