...
export function PromotedProductsRow() {
  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-[var(--gold)]/70 bg-[linear-gradient(135deg,var(--ink),#182b3a)] p-4 text-white shadow-[var(--shadow-ledger)] md:p-6">
      <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          <div className="grid h-12 w-12 shrink-0 place-items-center rounded-[1.1rem] border border-[var(--gold)] bg-[var(--gold)] text-[var(--ink)]"><Crown className="h-6 w-6" /></div>
          <div>
        ...
        <div className="inline-flex items-center gap-1 self-start rounded-full border border-white/15 bg-white/10 p-1">
          <button type="button" onClick={() => scroll("left")} aria-label="Scroll promoted products left" className="grid h-10 w-10 place-items-center rounded-full hover:bg-[var(--gold)] hover:text-[var(--ink)]"><ChevronLeft className="h-5 w-5" /></button>
          <span className="px-2 text-[0.65rem] font-black uppercase tracking-[0.12em] text-white/65">{promotions.length} promoted</span>
          <button type="button" onClick={() => scroll("right")} aria-label="Scroll promoted products right" className="grid h-10 w-10 place-items-center rounded-full hover:bg-[var(--gold)] hover:text-[var(--ink)]"><ChevronRight className="h-5 w-5" /></button>
        </div>
      </div>

      {error ? <p className="rounded-[1rem] border border-red-300/30 bg-red-500/15 p-3 text-sm font-bold text-red-100">{error}</p> : null}
      {notice ? <p className="mb-4 rounded-[1rem] border border-emerald-300/30 bg-emerald-500/15 p-3 text-sm font-bold text-emerald-100">{notice}</p> : null}
      ...
      {loading ? (
        ...
        <div ref={rowRef} className="flex snap-x snap-mandatory gap-4 overflow-x-auto overscroll-x-contain pb-4 [scrollbar-color:var(--gold)_transparent] [scrollbar-width:thin]">
          {promotions.map((promotion) => {
            ...
            return (
              <article key={promotion.id} className="flex w-[min(84vw,20rem)] shrink-0 snap-start flex-col overflow-hidden rounded-[1.55rem] border border-white/15 bg-white text-[var(--ink)] shadow-[0_18px_44px_rgba(0,0,0,0.25)] transition hover:-translate-y-1 hover:border-[var(--gold)]">
                ...
                <div className="flex flex-1 flex-col gap-4 p-4">
                  ...
                  <div className="mt-auto grid grid-cols-2 gap-2">
                    ...
                    <Link href={`/dashboard/user/shop/products/${product.id}`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-[var(--line)] bg-white px-4 text-sm font-black hover:border-[var(--gold)] hover:bg-[var(--surface)]"><Eye className="h-4 w-4" /> Details</Link>
                  </div>
                </div>
              </article>
            );
