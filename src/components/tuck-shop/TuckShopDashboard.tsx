...
async function loadCompleteCatalogue(search: string, businessId: string) {
  ...
  products.forEach((product) => uniqueProducts.set(product.id, product));
...
function BusinessProductSection({
  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-4 shadow-[var(--shadow-soft)] md:p-6">
      ...
      <div
        ref={productRowRef}
        className="flex snap-x snap-mandatory gap-4 overflow-x-auto overscroll-x-contain pb-4 pr-2 [scrollbar-color:var(--line-strong)_transparent] [scrollbar-width:thin]"
      >
