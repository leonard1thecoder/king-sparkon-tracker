...
import { CheckCircle2, Loader2, PackageCheck, Search, ShoppingCart, Store } from "lucide-react";
  ...
  return (
    <section className="grid gap-5">
      <SectionHeader
        ...
        description="Products are grouped by business so customers can clearly see which business owns each catalogue item, even after search or business filtering."
      />
      ...
      <Card>
        <CardHeader className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--steel)]">Search all active products. Filter results still remain grouped under each business.</p>
          </div>
          <form
          ...
          {loading ? (
            ...
            <div className="grid gap-6">
              {businessGroups.map((group) => (
                <section key={group.key} className="rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-4 shadow-[var(--shadow-soft)] md:p-5">
