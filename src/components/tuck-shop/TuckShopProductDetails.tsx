...
function isProductListResponse(
  value: unknown,
): value is Product[] {
  return Array.isArray(value);
}

function isPagedProductResponse(
  value: unknown,
): value is { content?: Product[] } {
...
export function TuckShopProductDetails({
  productId,
}: {
  productId: string;
}) {
    ...
    <main className="mx-auto w-full max-w-7xl px-4 py-6 md:px-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <Link
          ...
          className="inline-flex min-h-11 items-center gap-2 rounded-full border border-[var(--line)] bg-white px-4 text-sm font-black text-[var(--ink)] hover:border-[var(--gold)]"
        >
      ...
      {loading ? (
        <div className="h-[32rem] animate-pulse rounded-[2.4rem] border border-[var(--line)] bg-white" />
      ) : null}

      {!loading && error ? (
        <Card>
          <CardContent className="p-8 text-sm font-bold text-[var(--danger)]">
            {error}
          </CardContent>
        </Card>
      ) : null}
      ...
      {!loading && !error && !product ? (
        <Card>
          <CardContent className="p-10 text-center">
            <h1 className="text-3xl font-black text-[var(--ink)]">
              Product not found
            </h1>
          ...
          <Card className="overflow-hidden">
            ...
            <CardHeader>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="font-mono text-xs font-black uppercase tracking-[0.18em] text-[var(--signal)]">
                    Product details
                  </p>
                  ...
                  <p className="mt-2 inline-flex items-center gap-2 text-sm font-bold text-[var(--steel)]">
            ...
            <CardContent className="grid gap-4">
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-[1.25rem] border border-[var(--line)] bg-[var(--surface)] p-4">
                  ...
                  <p className="money mt-1 text-2xl font-black text-[var(--ink)]">
              ...
              <p className="text-sm leading-7 text-[var(--steel)]">
                Choose the quantity like ticket checkout, then add this
                product to the shared user cart.
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="h-fit">
            <CardHeader>
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--steel)]">
                Quantity is capped by current stock availability.
              </p>
            </CardHeader>

            <CardContent className="grid gap-5">
              {notice ? (
                <div className="flex items-center gap-2 rounded-[1.25rem] border border-[var(--confirm)]/30 bg-[var(--confirm)]/10 p-4 text-sm font-black text-[var(--ink)]">
              ...
              <div className="grid gap-2">
                ...
                <div className="flex min-h-14 items-center justify-between rounded-[1.35rem] border border-[var(--line)] bg-white px-3 shadow-[var(--shadow-soft)]">
                  <button
                    ...
                    className="grid h-10 w-10 place-items-center rounded-full border border-[var(--line)] text-[var(--ink)]"
                  >
                    <Minus className="h-4 w-4" />
                  </button>

                  <strong className="money text-3xl">
                    {quantity}
                  </strong>
                  ...
                  <button
                    ...
                    className="grid h-10 w-10 place-items-center rounded-full border border-[var(--line)] text-[var(--ink)] disabled:cursor-not-allowed disabled:opacity-40"
                  >
              ...
              <div className="rounded-[1.35rem] bg-[var(--ink)] p-5 text-white">
                <p className="font-mono text-xs font-black uppercase tracking-[0.16em] text-[var(--gold)]">
                  Line total
                </p>

                <p className="money mt-2 text-4xl font-black">
                  {money(lineTotal)}
                </p>
              </div>
              ...
              <Link
                href="/dashboard/user/shop/cart"
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-[var(--line)] bg-white px-5 text-sm font-black text-[var(--ink)] hover:border-[var(--gold)]"
              >
                Go to cart
              </Link>
            </CardContent>
