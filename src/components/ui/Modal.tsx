...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/28 p-4 backdrop-blur-[2px]" role="presentation" onMouseDown={onClose}>
      <div
        className="w-full max-w-xl overflow-hidden rounded-[var(--radius-2xl)] border border-[var(--line-strong)] bg-white shadow-[0_24px_60px_rgba(15,23,42,0.14)]"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-[var(--line)] bg-white px-6 py-5">
          <div>
            ...
            <h2 id="modal-title" className="mt-1.5 text-xl font-extrabold tracking-[-0.03em] text-[var(--ink)]">{title}</h2>
          </div>
